...
# Faktor zur Umrechnung von Jahreswerten auf den gewählten Zeitraum ermitteln
# Basis: "Monatlich" (Faktor 1/12), "Ganzes Jahr" (Faktor 1), "Gesamt" (nutzt Monate/Jahre je nach Text)
jahres_faktor = 1.0
...
if zeitraum_modus == "Monatlich":
  ...
  # Bei individuellem Gesamtzeitraum fragen wir die Jahre ab
...
kraftstoffpreis = st.number_input(
...
st.session_state["kraftstoffpreis_val"] = kraftstoffpreis

standard_verbrenner_verbrauch = (
    6.0 if selected_fuel_short == "Diesel" else 7.5
)
if (
    st.session_state["verbrauch_verbrenner_val"] == 7.5
...
if st.button("Ersparnis berechnen", type="primary", use_container_width=True):
  ...
  # Gesamtersparnis = (Verbrenner Kraftstoff + Steuerersparnis + THG-Erlös) - E-Auto Ladekosten
  ...
  # 5. Amortisation berechnen (Bezieht reine Betriebskosten- und Unterhaltsersparnis pro km ein)
